...
export default function SignupPage() {
      ...
      <div className="min-h-screen flex items-center justify-center bg-cream-100 p-4">
        <div className="w-full max-w-sm animate-scale-in">
          <div className="card-elevated p-8 text-center">
            ...
            <p className="text-sm text-ink-400 mb-1">We&apos;ve sent a confirmation link to</p>
            <p className="text-sm font-medium text-ink-700 mb-4">{email}</p>
            <p className="text-sm text-ink-400 leading-relaxed mb-6">Click the link to activate your account and start your longevity journey.</p>
            <Button variant="outline" className="w-full border-cream-300 hover:bg-cream-100 text-ink-600" onClick={() => router.push('/login')}>Back to sign in</Button>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-cream-100 p-4 relative overflow-hidden">
      <div className="blob-accent w-[400px] h-[400px] bg-sand-400 -top-32 -left-32 fixed" />
      <div className="blob-accent w-[300px] h-[300px] bg-forest-500 bottom-0 -right-24 fixed" />
      ...
      <div className="w-full max-w-sm animate-fade-up relative">
        ...
        <div className="card-elevated p-8">
          <div className="text-center mb-7">
            <h1 className="text-xl font-display font-semibold text-ink-800">Create your account</h1>
            <p className="text-sm text-ink-400 mt-1.5">Start your longevity journey</p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input id="password" type="password" placeholder="Minimum 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} className="w-full px-3.5 py-2.5 text-sm" />
            </div>
            <div className="space-y-1.5">
              <label htmlFor="confirmPassword" className="text-sm font-medium text-ink-600">Confirm password</label>
              <input id="confirmPassword" type="password" placeholder="Repeat your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required className="w-full px-3.5 py-2.5 text-sm" />
            </div>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center"><span className="bg-white px-3 text-xs text-ink-300 uppercase tracking-wider font-medium">or</span></div>
          </div>

          <Button type="button" variant="outline" className="w-full h-11 border-cream-300 hover:bg-cream-100 text-ink-600 font-medium" onClick={handleGoogleSignup} disabled={loading}>
        ...
        <p className="text-center text-sm text-ink-400 mt-6">
          ...
          <Link href="/login" className="text-forest-600 hover:text-forest-700 font-medium">Sign in</Link>
        </p>
      </div>
    </div>
  )
